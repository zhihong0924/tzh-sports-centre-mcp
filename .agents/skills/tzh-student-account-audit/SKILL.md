...
- State which tool ran and whether the outcome was read-only, draft, reviewed,
  or committed.
- If authentication fails because a credential expired, was revoked, or lacks
  `student-audit:manage`, ask the user to obtain an audit-enabled replacement
  from TZH. A points-only token cannot authorize this workflow. Never ask them
  to paste it into chat; TZH creates replacements in the website's MCP Access
  Tokens workspace and sends the plaintext separately through a secure channel.
